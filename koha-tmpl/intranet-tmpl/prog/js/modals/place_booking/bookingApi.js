...
export async function fetchBookableItems(biblionumber) {
    ...
    if (!response.ok) {
        const error = new Error(`Failed to fetch bookable items: ${response.status} ${response.statusText}`);
...
export async function fetchBookings(biblionumber) {
    ...
    const response = await fetch(`/api/v1/biblios/${encodeURIComponent(biblionumber)}/bookings`);
    
    if (!response.ok) {
        const error = new Error(`Failed to fetch bookings: ${response.status} ${response.statusText}`);
        error.status = response.status;
        throw error;
    }
...
export async function fetchCheckouts(biblionumber) {
    ...
    const response = await fetch(`/api/v1/biblios/${encodeURIComponent(biblionumber)}/checkouts`);
    
    if (!response.ok) {
        const error = new Error(`Failed to fetch checkouts: ${response.status} ${response.statusText}`);
        error.status = response.status;
        throw error;
    }
...
export async function fetchPatron(patronId) {
    ...
    if (!response.ok) {
        const error = new Error(`Failed to fetch patron: ${response.status} ${response.statusText}`);
        error.status = response.status;
        throw error;
    }
...
export async function fetchPatrons(term, page = 1) {
    ...
    if (!response.ok) {
        const error = new Error(`Failed to fetch patrons: ${response.status} ${response.statusText}`);
        error.status = response.status;
...
export async function fetchPickupLocations(biblionumber, patronId) {
    ...
    if (!response.ok) {
        const error = new Error(`Failed to fetch pickup locations: ${response.status} ${response.statusText}`);
        error.status = response.status;
        throw error;
    }
...
export async function fetchCirculationRules(params = {}) {
    ...
    if (!response.ok) {
        const error = new Error(`Failed to fetch circulation rules: ${response.status} ${response.statusText}`);
        error.status = response.status;
        throw error;
    }
...
export async function createBooking(bookingData) {
    ...
    const requiredFields = ['start_date', 'end_date', 'biblio_id', 'patron_id', 'pickup_library_id'];
...
export async function updateBooking(bookingId, bookingData) {
    ...
    const response = await fetch(`/api/v1/bookings/${encodeURIComponent(bookingId)}`, {
        method: "PUT",
        headers: { 
            "Content-Type": "application/json",
            "Accept": "application/json"
