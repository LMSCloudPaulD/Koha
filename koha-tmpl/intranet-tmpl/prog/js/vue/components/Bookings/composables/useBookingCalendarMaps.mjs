...
import { BookingDate, isoArrayToDates } from "../lib/booking/BookingDate.mjs";
import { buildIntervalTree } from "../lib/booking/algorithms/interval-tree.mjs";
import {
    buildUnavailableByDateMap,
    addHolidayMarkers,
} from "../lib/booking/availability/unavailable-map.mjs";
import { calculateMaxEndDate } from "../lib/booking/availability/period-validators.mjs";
import { createDisableFunction } from "../lib/booking/availability/disabled-dates.mjs";
import { findFirstBlockingDate } from "../lib/booking/availability/date-change.mjs";
import {
    extractBookingConfiguration,
    toEffectiveRules,
} from "../lib/booking/availability/rules.mjs";
import { getBookingMarkersForDate } from "../lib/booking/markers.mjs";
import {
...
export function useBookingCalendarMaps({
    ...
    const disabledByDate = computed(() => {
        ...
        if (isEndDateOnly && anchor && maxPeriod && maxPeriod > 0) {
            ...
            for (
                ...
                result.set(key, {
                    reason: "Intermediate date in fixed range",
                    severity: "soft",
                });
            }
        }
    ...
    const loanBoundaryTimes = computed(() => {
        ...
        if (issuelength > 0) {
            result.add(
                startDate.add(issuelength, "day").toDate().getTime()
            );
    ...
    function rangePreviewFn(anchor, hover) {
        ...
        if (maxPeriod && days > maxPeriod) {
            return {
                status: "invalid",
                message: `Range exceeds max booking period (${maxPeriod} days)`,
            };
        }
        ...
        for (
            ...
            if (fn(d.toDate())) {
                return {
                    status: "invalid",
                    message: `Range includes blocked day`,
                };
            }
        }
