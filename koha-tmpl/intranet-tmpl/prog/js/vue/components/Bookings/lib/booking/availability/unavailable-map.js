...
import { BookingDate, addDays, subDays } from "../BookingDate.js";
import { SweepLineProcessor } from "../algorithms/sweep-line-processor.js";
import {
 ...
 * @param {import('../algorithms/interval-tree.js').IntervalTree} intervalTree - The interval tree containing all bookings/checkouts
...
export function addLeadPeriodFromTodayMarkers(
    ...
    for (let i = 0; i < leadDays; i++) {
        ...
        allItemIds.forEach(itemId => {
            ...
            if (
                existing &&
                (existing.has("booking") || existing.has("checkout"))
            ) {
                return;
            }
            if (!existing) {
 ...
 *
 * @param {import('../../../types/bookings').UnavailableByDate} unavailableByDate - The map to modify
 * @param {import('../algorithms/interval-tree.js').IntervalTree} intervalTree - The interval tree with all bookings/checkouts
 * @param {import('dayjs').Dayjs} today - Today's date
 * @param {number} leadDays - Number of lead period days
