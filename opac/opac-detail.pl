...
if ($OpacBrowseResults) {
    ...
    if ( $session->param('busc') ) {
        ...
        sub searchAgain {
            ...
            eval {
                ...
                ( $error, $results_hashref, $facets ) = $searcher->search_compat(
                    $json->decode( $arrParamsBusc->{'query'} ),
                    $arrParamsBusc->{'simple_query'}, \@sort_by, \@servers, $results_per_page, $offset, undef,
                    $itemtypes, $arrParamsBusc->{'query_type'}, $arrParamsBusc->{'scan'}
                );
            };
            my $hits;
            my @newresults;
            ...
            for ( my $i = 0 ; $i < @servers ; $i++ ) {
                ...
                @newresults = searchResults(
                    $search_context, '', $hits, $results_per_page, $offset, $arrParamsBusc->{'scan'},
        ...
        my $results_per_page =
            ( $arrParamsBusc{'count'} && $arrParamsBusc{'count'} =~ /^[0-9]+?/ ) ? $arrParamsBusc{'count'} : $count;
        $arrParamsBusc{'count'} = $results_per_page;
        my $offset =
            ( $arrParamsBusc{'offset'} && $arrParamsBusc{'offset'} =~ /^[0-9]+?/ ) ? $arrParamsBusc{'offset'} : 0;
            ...
            if (@arrBiblios) {
                ...
                if ( $arrBiblios[0] == $biblionumber ) {
                    ...
                    if ( !$offset ) {
                        ...
                        $searchAgain = 1
                            if ( !$arrParamsBusc{'next'} && $arrParamsBusc{'total'} > ( $offset + @arrBiblios ) );
                    }
        ...
        for ( my $j = 0 ; $j < @arrBiblios ; $j++ ) {
            ...
            push @listResults,
                {
                index  => $j + 1 + $offset, biblionumber => $arrBiblios[$j],
                title  => ( $arrBiblios[$j] == $biblionumber ) ? '' : $dataBiblioPaging->title,
                author => ( $arrBiblios[$j] != $biblionumber && $dataBiblioPaging->author )
                ? $dataBiblioPaging->author
                : '', part_number => ( $arrBiblios[$j] == $biblionumber ) ? '' : $dataBiblioPaging->part_number,
                part_name => ( $arrBiblios[$j] == $biblionumber ) ? '' : $dataBiblioPaging->part_name,
                url => ( $arrBiblios[$j] == $biblionumber ) ? '' : 'opac-detail.pl?biblionumber=' . $arrBiblios[$j]
                };
        }
        $template->param( 'listResults' => \@listResults ) if (@listResults);
        $template->param(
            'indexPag'    => 1 + $offset, 'totalPag' => $arrParamsBusc{'total'},
...
my $shelflocations = {
    map { $_->{authorised_value} => $_->{opac_description} } Koha::AuthorisedValues->get_descriptions_by_koha_field(
        { frameworkcode => $dat->{frameworkcode}, kohafield => 'items.location' }
    )
};
my $collections = {
    map { $_->{authorised_value} => $_->{opac_description} } Koha::AuthorisedValues->get_descriptions_by_koha_field(
        { frameworkcode => $dat->{frameworkcode}, kohafield => 'items.ccode' }
    )
};
my $copynumbers = {
    map { $_->{authorised_value} => $_->{opac_description} } Koha::AuthorisedValues->get_descriptions_by_koha_field(
        { frameworkcode => $dat->{frameworkcode}, kohafield => 'items.copynumber' }
    )
};
...
if ( C4::Context->preference('LibraryThingForLibrariesEnabled') ) {
    ...
    $template->param(
        LibraryThingForLibrariesTabbedView => C4::Context->preference('LibraryThingForLibrariesTabbedView') );
}
...
my $defaulttab =
      $viewallitems                                                                      ? 'holdings'
    : $opac_serial_default eq 'subscriptions' && $subscriptionsnumber                    ? 'subscriptions'
    : $opac_serial_default eq 'titlenotes' && $subscriptionsnumber                       ? 'descriptions'
    : $opac_serial_default eq 'serialcollection' && @serialcollections > 0               ? 'serialcollection'
    : $opac_serial_default eq 'holdings' && scalar(@itemloop) > 0                        ? 'holdings'
    : ( $showcomp eq 'both' || $showcomp eq 'opac' ) && scalar(@itemloop) == 0 && $parts ? 'components'
    : scalar(@itemloop) == 0                                                             ? 'media'
...
if ( C4::Context->preference('OPACAuthorIdentifiersAndInformation') ) {
    ...
    for my $author ( @{ $biblio->get_marc_authors } ) {
        ...
        my ($name) =
            map { $_->{value} }
            grep { $_->{code} eq 'a' ? $_ : () } @{ $author->{MARCAUTHOR_SUBFIELDS_LOOP} };
