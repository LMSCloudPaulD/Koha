...
describe("Booking Modal Timezone Tests", () => {
    ...
    it("should correctly span multi-day bookings without timezone-induced extra days", () => {
        ...
        cy.task("insertSampleBooking", {
            ...
            patron: testData.patron,
            pickup_library_id: testData.libraries[0].library_id,
            start_date: bookingStart.startOf("day").toISOString(),
            end_date: bookingEnd.endOf("day").toISOString(),
        });
    ...
    it("should correctly detect conflicts using timezone-aware date comparisons", () => {
        ...
        cy.task("insertSampleBooking", {
            ...
            patron: testData.patron,
            pickup_library_id: testData.libraries[0].library_id,
            start_date: existingStart.startOf("day").toISOString(),
            end_date: existingEnd.endOf("day").toISOString(),
        });
    ...
    it("should correctly round-trip dates through API without timezone shifts", () => {
        ...
        // Actually submit and assert the API payload carries the same local
        // calendar days — the round-trip this test is named for.
        ...
        cy.wait("@createBooking").then(({ request, response }) => {
            ...
            expect(dayjs(request.body.start_date).format("YYYY-MM-DD")).to.eq(
                startDate.format("YYYY-MM-DD")
            );
            expect(dayjs(request.body.end_date).format("YYYY-MM-DD")).to.eq(
                endDate.format("YYYY-MM-DD")
            );
        });
    });
